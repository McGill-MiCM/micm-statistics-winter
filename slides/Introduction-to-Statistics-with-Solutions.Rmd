---
title: "Introduction to Statistics in R (With Solutions)"
...
```{r}
# The idea is to only check a few values. The student can chose their favorites.

dbinom(c(-1, 0.5, pi), size = 1, prob = 0.2)
```
...
```{r warning = FALSE}
# Let's create the grid values. The by parameter can be chosen by the student.
x_bernoulli <- seq(-1, 1, by = 0.01)

# Evalue the grid points on the pmf
pmf_bernoulli <- dbinom(x_bernoulli, size = 1, prob = 0.2)

# Plot the pmf values
plot(x_bernoulli, pmf_bernoulli, 
     pch = 16, 
     col = "chartreuse4", 
     xlab = "x", ylab = "pmf")
```
...
```{r}
# Two possibilities for this. Either do
1 - pbinom(50, size = 100, prob = 0.066)

# or do
pbinom(50, size = 100, prob = 0.066, lower.tail = FALSE)

# We can check if they're the same thing
(1 - pbinom(50, size = 100, prob = 0.066)) == pbinom(50, size = 100, prob = 0.066, lower.tail = FALSE)
```
...
# or do
ppois(0, lambda = 30, lower.tail = FALSE)

# We can check if they're the same thing
1 - ppois(0, lambda = 30) == ppois(0, lambda = FALSE, lower.tail = FALSE)
```
...
We don't know what is the density function but we can try to estimate it using the $\texttt{hist}$ function in R. Let us obtain 50 samples from this unknown distribution.
```{r}
set.seed(1)
...
A random variable $X$ has normal distribution with parameters $\mu$ and $\sigma^2$ with $\mu \in \R$ and $\sigma > 0$ if its pdf can be written as
\begin{equation*}
...
* The value $x$ in which we want to evaluate the pdf or cdf.
...
```{r}
pnorm(1500, mean = 1350, sd = 150) - pnorm(1200, mean = 1350, sd = 150)
```

2. A brain was measured in a recently diseased patient and it was found its volume was $1800 \text{cm}^3$. The doctor who measured feels something must have been wrong with the patient. How likely it is to find a brain size like that one or bigger by pure chance?

```{r}
pnorm(1800, mean = 1350, sd = 150, lower.tail = FALSE)
```
3. Simulate a sample of size 50 of brain sizes. Save the results in a vector called $\texttt{brain_samples}$. Plot a normalized histogram of this sample. Use the following code to overlay the true density of the sample:
```{r eval = FALSE}
# Create a grid of x values
x_brains_density <- seq(floor(min(brain_samples)), ceiling(max(brain_samples)), by = 1)
...
```{r}
brain_samples <- rnorm(n = 50, mean = 1350, sd = 150)

hist(brain_samples, freq = FALSE, col = "chartreuse4")
...
# Overlay histogram with fitted density
lines(x_brains_density, y_brains_density, col = "red", lwd = 2)
```
...
\begin{equation}
  ...
  \int_{-\infty}^{+\infty} (x-\E(X))^2 p_X(x) \,dx  & \text{if } X \text{ is continuous.}
\end{cases}
\end{equation}
...
1. A [CpG site](https://en.wikipedia.org/wiki/CpG_site) is a region of DNA where a cytosine is followed by a guanine or viceversa. From a starting point in a linear sequence the length of a sequence until the occurrence of a CpG site was measured. This can be modeled with a [geometric distribution](https://en.wikipedia.org/wiki/Geometric_distribution) of parameter $p$. The first column of the data set $\texttt{mean-variance.csv}$ gives us the length of a non-CpG site until a CpG site is found. Find the sample mean and sample variance to estimate $p$. Make a histogram of the dataset.

```{r}
mean_variance <- read.csv("C:/Users/Gerardo/Downloads/mean-variance.csv", header=FALSE)

# Mean is
mean(mean_variance[,1])
```
```{r}
# In a geometric distribution, E(X) = 1/p. Then p is
1/mean(mean_variance[,1])
```
```{r}
hist(mean_variance[,1], main = "", freq = FALSE, xlab = "x")
```

2. When modelling *count data*, that is, data that comes from an underlying model that counts the number of events of a certain type, a Poisson distribution is usually used. However, as we have seen, the Poisson distribution has equal mean and variance. In the second column of $\texttt{mean-variance.csv}$ data we have the number of counts of the expression of the gene NeuN in neurons. Check if the mean is equal to the variance. As you won´t necessarily get *the exact* same number, we will say the mean and variance are equal if the distance between them is less than $0.5$.

```{r}
mean(mean_variance[,2])
var(mean_variance[,2])
```
...
We have been hired to do quality control at a factory that produces dice for a board game in which getting a $6$ is extremely important. We know don't want to ruin the players' experience so if a die is not a fair die we want to discard the die. Given a particular die we construct two hypothesis:
...
The company is very environmentally friendly so we don't want to throw unnecessary dice. We would take an incorrect decision if the die is fair but we decide to throw it. Given a perfectly good die, we want to estimate how frequently we would throw incorrectly these dice.
...
We are now ready to estimate the probability of throwing away a perfectly good die.
```{r}
...
```
We can now calculate the probability of not throwing the loaded die away.
```{r eval = FALSE}
(sum(fair_die_number_sixes<=25))/1000
```
...
\end{equation*}
if and only if $F_T(k) \leq 0.025$. Hence, we want to find $k$ such that $F_T(k) \leq 0.025$.
...
We can now go back to our data and check if we will reject the null hypothesis. Fortunately for us, there is a function in R called $\texttt{t.test}$ that does everything for us.
```{r eval = FALSE}
